...
if __name__ == '__main__':
    ...
    parser.add_argument("--dyn_sys", default="lorenz", choices=["lorenz", "rossler", "baker", "tilted_tent_map", "plucked_tent_map", "pinched_tent_map", "KS"])
    ...
    if args.model_type == "MLP":
        ...
        m = FNO1d(2, width=32, layers=4, fc_dim=128, in_dim=3, out_dim=3)
    ...
    # if (str(args.dyn_sys) == "pinched_tent_map") or (str(args.dyn_sys) == "tilted_tent_map") or (str(args.dyn_sys) == "plucked_tent_map"):      
    ...
    #         else:
    #             for t in range(x.shape[0]):
    #                 le += torch.log(abs(F.jacobian(dyn_sys_func, x[t].to(device))))
    #         return le/T
