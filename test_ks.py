    ...
    for i in range(epochs):
        ...
        if batch_size != None:
            ...
            for batch_start in range(0, num_train, batch_size):
                ...
                y_pred[batch_idx] = model(X_batch.reshape(batch_size, 1, -1)).to(device)
...
def plot_vf_err(model, dyn_info, model_type, loss_type, vf_err_path):
    dyn, dim, time_step = dyn_info
...
if __name__ == '__main__':
    ...
    # grid search
    modelchoices = ['FNO']#['MLP','MLP_skip']
    hiddenchoices = [256]#[256, 512, 1024]
    layerchoices = [5]#[3, 5, 7]
    ...
    parser.add_argument("--model_type", default="FNO", choices=["MLP","MLP_skip", "CNN", "HigherDimCNN", "GRU", "FNO"])
    ...
    for index, combination in enumerate(combinations):
        ...
        if args.model_type == "MLP":
            ...
            m = Net2d(in_dim=3, out_dim=3, domain_size=64, modes=20, width=128).to(device)
